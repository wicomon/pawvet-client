...
import { PLAN_INTERVAL_LABEL, getSubscriptionState, formatDate, formatMoney } from "@/lib/billing";
...
export default function SubscriptionSummaryCard({
  ...
  const canCancel = subscription && !subscription.cancelAtPeriodEnd;
          ...
          return (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span
                  className={`w-fit whitespace-nowrap rounded-full px-2.5 py-1 text-[11px] font-extrabold ${state.badge}`}
                >
                  {state.label}
                </span>
                {state.note && (
                  <span className="text-[12.5px] font-semibold text-wv-muted-2">{state.note}</span>
                )}
              </div>
