...
export default function SubmitButton({ label, pendingLabel, pending }: SubmitButtonProps) {
  return (
    <button
      type="submit"
      disabled={pending}
      className="flex h-[46px] w-full items-center justify-center gap-2.5 rounded-[11px] bg-wv-teal text-[14.5px] font-extrabold text-white outline-none transition-[background-color,transform] duration-150 ease-out hover:bg-wv-teal-hover focus-visible:shadow-focus active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70"
    >
      {pending && (
